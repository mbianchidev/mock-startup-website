...
GitHub Actions validates the application and deploys the static Next.js export to
Vercel.

- Validation workflow: `.github/workflows/ci.yml`
- Deployment workflow: `.github/workflows/vercel.yml`
- Output directory: `out/` (generated by `next build` with `output: 'export'`)
- Successful pull request validation runs receive Vercel preview deployments.
- Successful pushes to `main` deploy to production.

Configure these GitHub Actions repository secrets before running deployment jobs:

- `VERCEL_TOKEN`
- `VERCEL_ORG_ID`
- `VERCEL_PROJECT_ID`

Assign `mbianchi.dev` to the Vercel project and configure the domain's DNS records
using the values Vercel provides. After the production deployment and domain are
verified, disable the previous GitHub Pages deployment in the repository settings.
